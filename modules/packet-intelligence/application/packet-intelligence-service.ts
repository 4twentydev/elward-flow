...
export class PacketIntelligenceService {
  ...
  ingestDocument(
    ...
    const doc: JobPacketDocument = {
      ...
      fileStorageKey: params.fileStorageKey,
  ...
  listDocuments(session: SessionContext, jobId?: string): JobPacketDocument[] {
    return Array.from(this.documents.values()).filter(
      (d) => d.organizationId === session.activeOrganization.id && (!jobId || d.jobId === jobId)
    );
  }
}
