...
export class QualityService {
  ...
  constructor() {
    this.seedDefaultRecords("org_default");
  }

  private seedDefaultRecords(orgId: string) {
    const defaultNcr: NonConformanceReport = {
      id: "ncr_42",
      ...
      operationName: "WC-BRAKE-01 // Sheet Metal Forming",
      defectDescription: "Return flange angle measured at 92.5° exceeding ±0.5° drawing tolerance.",
      defectCategory: "Dimensional Variance",
      severity: "major",
      status: "open",
      defectQuantity: 8,
      containmentActions: ["Segregated 8 out-of-spec parts to red QA quarantine holding bin."],
      disposition: "pending_review",
      estimatedCostImpact: {
        scrapCost: 140.0,
        reworkLaborHours: 1.5,
        remakeCost: 0,
      },
      createdByUserId: "usr_inspector",
      createdByName: "Lead QA Inspector",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };

    this.ncrs.set(defaultNcr.id, defaultNcr);
  }
  ...
  createNCR(
    ...
    const ncr: NonConformanceReport = {
      ...
      disposition: "pending_review",
      estimatedCostImpact: {
        scrapCost: params.estimatedScrapCost || 0,
        reworkLaborHours: params.estimatedReworkHours || 0,
        remakeCost: 0,
      },
  ...
  applyDisposition(
    ...
    if (params.finalScrapCost !== undefined) {
      ncr.estimatedCostImpact.scrapCost = params.finalScrapCost;
    }
  ...
  getQualityMetrics(session: SessionContext): QualityMetricsSummary {
    ...
    const scrapValuationLoss = orgNcrs.reduce((acc, n) => acc + n.estimatedCostImpact.scrapCost, 0);
