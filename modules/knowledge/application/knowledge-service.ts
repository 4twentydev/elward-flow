...
export class KnowledgeService {
  ...
  searchAndAnswerQuery(session: SessionContext, queryText: string): KnowledgeQueryResult {
    ...
    for (const art of orgArticles) {
      ...
      if (art.title.toLowerCase().includes(query)) score += 0.5;
      if (art.tags.some((t) => query.includes(t) || t.includes(query))) score += 0.4;
      if (rev.content.toLowerCase().includes(query)) score += 0.3;

      let bestStep: ProcedureStep | undefined;
      for (const step of rev.structuredSteps) {
        if (step.instruction.toLowerCase().includes(query) || step.title.toLowerCase().includes(query)) {
          score += 0.35;
          bestStep = step;
        }
      }
    ...
    const citedPassage = top.bestStep
      ? `Step ${top.bestStep.stepNumber}: ${top.bestStep.instruction}`
      : topRev.content;
