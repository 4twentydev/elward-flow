...
export class PurchasingService {
  ...
  recordReceipt(
    ...
    for (const item of params.itemsReceived) {
      ...
      // Post immutable ledger transaction to inventory
      try {
        ...
        // If not found in catalog, register and receive
        inventoryService.createItem(session, {
          ...
          reorderPoint: 5,
          reorderQuantity: 10,
          standardCost: line.unitCostCents / 100,
        });
        inventoryService.receiveMaterial(session, {
          itemId: item.itemCode,
          quantity: item.quantityReceived,
  ...
  getSupplierOptions(itemCode: string): SupplierOption[] {
    return [
      {
        vendorId: "vend_1",
        vendorName: "Ryerson Metal Solutions",
  ...
  listShortages(session: SessionContext): MaterialShortageSignal[] {
    this.ensurePurchasingSeeded(session.activeOrganization.id);
    return Array.from(this.shortages.values());
  }
  ...
  getPurchasingMetrics(session: SessionContext): PurchasingMetricsSummary {
    ...
    return {
      ...
      openShortagesCount: this.shortages.size,
