...
describe("Analytics & Operational Planning Module", () => {
  ...
  it("compiles the needs-attention queue aggregating cross-module blocker exceptions", () => {
    const { user: owner } = identityService.bootstrapOwner({
      email: "owner@yorkstead.com",
      name: "Owner",
      organizationName: "Factory A",
    ...
    expect(dashboard.needsAttentionQueue.length).toBeGreaterThan(0);
    const shortageItem = dashboard.needsAttentionQueue.find((i) => i.module === "purchasing");
    expect(shortageItem).toBeDefined();
    expect(shortageItem?.constraints.length).toBeGreaterThan(0);
    expect(shortageItem?.tradeOffs.length).toBeGreaterThan(0);
  });
