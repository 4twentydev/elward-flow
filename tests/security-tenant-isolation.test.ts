...
describe("Security & Tenant Isolation Hardening Suite", () => {
  describe("Tenant Boundary & IDOR Prevention", () => {
    it("strictly isolates inventory balances between distinct organizations", () => {
      const identityServiceA = new IdentityService();
      const identityServiceB = new IdentityService();
      const inventoryService = new InventoryService();

      const { user: userA } = identityServiceA.bootstrapOwner({
      ...
      const { user: userB } = identityServiceB.bootstrapOwner({
        email: "owner_b@yorkstead.com",
        name: "Owner B",
        organizationName: "Factory Beta",
        organizationSlug: "factory-beta",
      });

      const sessionA = identityServiceA.getSessionContext(userA.id);
      const sessionB = identityServiceB.getSessionContext(userB.id);
      ...
      const itemB = inventoryService.createItem(sessionB, {
        ...
        category: "raw_material",
        unitOfMeasure: "SHEET",
        defaultLocationCode: "RACK-B",
        reorderPoint: 5,
        reorderQuantity: 20,
        standardCost: 45.0,
      });

      // Receive 100 into Org A and 25 into Org B
    ...
    it("rejects cross-tenant overdraft issues", () => {
      ...
      // Attempting to issue 50 from Org B (which only has 25) must fail with negative stock overdraft error
      expect(() => {
    ...
    it("sanitizes potentially dangerous CSV formula prefixes (=, +, -, @)", () => {
      function sanitizeCsvCell(cell: string): string {
        ...
        if (cell && dangerousPrefixes.some((p) => cell.startsWith(p))) {
          return `'${cell}`; // Prepend single quote to neutralize spreadsheet execution
        }
        return cell;
      }
  ...
  describe("File Vault Path Traversal Containment", () => {
    it("rejects path traversal keys attempting to escape root directory", () => {
      function validateFileKey(key: string): boolean {
        // Must be alphanumeric, dashes, underscores, and forward slashes without .. traversal
